...
**Debug Logging:** Three-tier system: `Logger` (user-facing, in-memory, Settings > Logs), `DebugLog` (legacy, still exists in `src/utils/DebugLog.ts`), and `UltraDevLog` (primary, 3,000-entry memory + file-based JSONL). UltraDevLog v3 is a complete superset of DebugLog with 100+ methods including 54 legacy compatibility stubs. All instrumented files import `UltraDevLog as DebugLog` — zero call-site changes needed. **Three log directories on device:** (1) `ultra_dev_logs/` — UltraDevLog direct writes via doFlush() every 2s (fixed filename `session_{id}.jsonl`), (2) `debug_logs/` — DebugLog direct writes via flushToFile() every 3s (fixed filename `debug_{id}.jsonl`), (3) `agent-ultra-logs/` — LogFolder directory, written by UltraDevLog.doFlush() and DebugLog.flushToFile() and exportAll(). Settings Logs tab reads from `agent-ultra-logs/` via LogFolder.listLogs(). **Fixed filenames (v3.20.0):** (1) `ultra-devlog.jsonl` from UltraDevLog, (2) `debug-log.jsonl` from DebugLog auto-flush, (3) `raw-export.jsonl` from manual export. Each flush overwrites the same file using `writeAsStringAsync`, so files grow naturally (~600KB for Ultra, ~2.2MB for Debug, max ~5MB total). Old timestamped files are cleaned up on app startup. **File system import:** Must use `expo-file-system/legacy` — SDK 54 removed `writeAsStringAsync` from the main export. **v3 sensors (all instrumented):** COMPONENT_LIFECYCLE, SETTINGS_SAVE, EXECUTE_PHASE, NAV_CHANGE, FOCUS_EFFECT_DEPS, PROCESS_RESTART, PICKER_CONTENT, CONTEXT_PROVIDER. v2 sensors: CORE_INSTANCE, EXECUTOR_BRANCH, CONV_CONTEXT_SENT, UI_MESSAGE_RENDERED, TASK_WATCHDOG, APP_STATE_CHANGE. `generateBugReport()` produces structured diagnostic reports. **Logs tab crash fixed (v3.19.0):** Three bugs — `loadDebugLogs()` undefined, `setImmediate` missing in RN, dead state variables. **metro.config.js must NOT exist** — breaks EAS builds on Windows CI. **Comprehensive bug reports:** `AGENT-ULTRA-BUG-REPORT.md` (full cross-linkage), `LOG-FILE-PROLIFERATION-REPORT.md` (log file analysis).

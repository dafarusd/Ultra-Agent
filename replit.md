...
**Debug Logging:** Three-tier system: `Logger` (user-facing, in-memory, Settings > Logs), `DebugLog` (legacy, still exists), and `UltraDevLog` (primary, 3000-entry memory + file-based JSONL in `ultra_dev_logs/`). UltraDevLog v3 is a complete superset of DebugLog with 100+ methods including 54 legacy compatibility stubs. All instrumented files import `UltraDevLog as DebugLog` — zero call-site changes needed. **v3 sensors (all instrumented):** COMPONENT_LIFECYCLE (mount/unmount with IDs in ChatScreen), SETTINGS_SAVE (tap/result wrapping api/defaults/limits saves), EXECUTE_PHASE (9-step phase tracking in AgentCore), NAV_CHANGE (navigation state listener in RootLayout), FOCUS_EFFECT_DEPS (dependency diff + suppression tracking in useFocusEffect), PROCESS_RESTART (Android process kill detection via AsyncStorage timestamp), PICKER_CONTENT (FlatList first-render diagnostic in ModelPickerSheet), CONTEXT_PROVIDER (reserved for future Context migration). v2 diagnostic sensors: CORE_INSTANCE, EXECUTOR_BRANCH, CONV_CONTEXT_SENT, UI_MESSAGE_RENDERED, TASK_WATCHDOG, APP_STATE_CHANGE. Existing categories: APP_LAUNCH_*, SMS_*, PICKER_*, UI_PROCESSING, UI_SEND_ATTEMPT/COMPLETE, UI_MODAL, PARSE_INPUT/PARSE_COMPOUND, VAULT_READ/WRITE. `generateBugReport()` produces structured diagnostic reports. Settings > Logs has Bug Report (orange) and Full Log (green) buttons. **v3 data quality fixes:** vaultGet passes boolean+preview instead of raw value; settingsApiSave/Delete/DefaultPick signatures corrected; pickerAnimate mountedAt guard replaces hasBeenVisible ref.

...
**Debug Logging:** Three-tier system: `Logger` (user-facing, in-memory, Settings > Logs), `DebugLog` (legacy, still exists), and `UltraDevLog` (primary, 3000-entry memory + file-based JSONL in `ultra_dev_logs/`). UltraDevLog is a complete superset of DebugLog with 70+ methods. All 11 instrumented files import `UltraDevLog as DebugLog` — zero call-site changes needed. New diagnostic categories: APP_LAUNCH_* (5-stage trace with flushSync before IntentLauncher), SMS_* (resolve/fire/result), PICKER_* (open/close/animate/select), UI_PROCESSING (isProcessing transitions), UI_SEND_ATTEMPT/COMPLETE, UI_MODAL (open/close tracking), PARSE_INPUT/PARSE_COMPOUND (command parsing), VAULT_READ/WRITE. `generateBugReport()` produces structured diagnostic reports. Settings > Logs has Bug Report (orange) and Full Log (green) buttons.
**Idempotency:** ExecutionLedger prevents duplicate actions, but `app_launch`, `camera_capture`, `media_access`, `device_location`, and `contacts_read` are exempt (repeatable capabilities). AgentCore init runs exactly once via a ref guard — no re-initialization on app foreground return.
